...
def update_output_container(input_year, selected_statistics):
    if selected_statistics == "Recession Period Statistics":
        ...
        def set_bin(x: float, range: list) -> str:
            """
            Set the bins for unemployment rate
            """
            for i in range:
                if x <= i:
                    return f"{i-1}%-{i}%"
        ...
        unemployment_data.loc[:, "unemployment_rate"] = unemployment_data.loc[
            :, "unemployment_rate"
        ].apply(lambda x: set_bin(x, range(0, 7)))
        unemployment_data.sort_values(by=["unemployment_rate"], inplace=True)
        R_chart4 = dcc.Graph(
            figure=px.bar(
                unemployment_data,
                x="unemployment_rate",
    ...
    # Yearly Statistic Report Plots
        ...
        Y_chart2 = dcc.Graph(
            figure=px.line(
                ...
                x="Month",
                y="Automobile_Sales",
                labels={"Month": "Month", "Automobile_Sales": "Total Sales Volume"},
